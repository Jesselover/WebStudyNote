...
区别：
1. nth—child对父元素里面所有孩子排序选择（序号是固定的）先找到第n个孩子，然后看看是否和E匹配%%先看数字,再看类型%%
2. nth—of—type对父元素里面指定子元素进行排序选择。先去匹配E ，然后再根据E找第n个孩子%%先看类型再看数字%%
...
- 转换可以理解为变形。
	...
	- 缩放：`scale`,`transform-origin`
-  二维坐标系
	注意：网页中的坐标系与现实中坐标系的区别

- 总结:
	- 转换transform 我们简单理解就是变形有2D和3D之分  
	- 我们暂且学了三个 分别是位移旋转和缩放  
	- 2D 移动translate(x, y)最大的优势是不影响其他盒子，里面参数用% ,是相对于自身宽度和高度来计算的 ,可以分开写比如 translateX(x)和translateY(y)  
...
- 还可以给 x y 设置像素(精确定单位)或者方位名词（top bottom left right center）
...
- `scale` 的优势：不占空间=>不会影响其他盒子
%%直接修改width,height可能会挤开其他盒子,scale则不会,类似移动translate%%
...
|`animation-timing-function`|规定动画的速度曲线，默认是“ease” ."infinite"匀速
|`animation-delay`|规定动画何时开始，默认是0.|
|`animation-iteration-count`|规定动画被播放的次数，默认是1，还有infinite. |
|`animation-direction`|规定动画是否在下一周期逆向播放，默认是 "normal",alternate逆播放.reverse动画由尾到头运行|
|`animation-play-state`|规定动画是否正在运行或暂停。默认是"running",还有"paused".|
|`animation-fill-mode`|规定动画结束后状态,保持forwards,回到起始(默认)backwards|

### 4.3 动画简写属性

```css
animation: 动画名称 持续时间 运动曲线 何时开始 播放次数 是否反方向 动画起始或者结束的状态(fill-mode);
```
...
|-|-|
|linear|匀速|
|ease|默认:低俗开始,然后加快,在结束时变慢|
|ease-in|以低速开始|
|ease-out|以低速结束|
|ease-in-out|以低速开始且以低速结束|
